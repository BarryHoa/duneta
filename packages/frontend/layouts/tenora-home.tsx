...
const foundations = [
  ['01', 'Web', 'Next.js application with a deliberately thin app layer.', 'apps/web'],
  ['02', 'API', 'Hono API, ESM-first and ready for Node or Workers.', 'apps/api'],
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <section id="top" className="grid gap-12 pb-20 pt-20 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:pt-28">
          <div>
            ...
            <p className="mt-7 max-w-xl text-lg leading-8 text-slate-400">A TypeScript starter where the Next.js product, Hono API, and publishable Tenora packages move as one.</p>
            ...
            <div className="mt-10 flex flex-wrap gap-x-6 gap-y-3 text-sm text-slate-400">{['Next.js 16', 'Hono API', 'Cloudflare ready', 'ESM packages'].map((item) => <span key={item} className="flex items-center gap-2"><Check />{item}</span>)}</div>
          </div>
          ...
          <Card className="hero-console border border-white/10 bg-slate-950/60 shadow-2xl shadow-cyan-950/30">
            ...
            <Card.Content className="space-y-5 p-5 sm:p-7">
              ...
              <div className="grid grid-cols-3 gap-3">{[['WEB', 'Next.js'], ['API', 'Hono'], ['UI', 'HeroUI']].map(([label, item]) => <div key={label} className="rounded-xl border border-white/8 bg-white/[0.035] p-3"><p className="text-[10px] font-semibold tracking-[0.14em] text-slate-500">{label}</p><p className="mt-2 text-sm font-medium text-slate-200">{item}</p></div>)}</div>
