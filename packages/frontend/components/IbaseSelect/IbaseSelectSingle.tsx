...
import type {
  IbaseSelectSingleOption,
  IbaseSelectSingleProps,
  IbaseSelectSingleSearchRule,
  IbaseSelectSingleVirtualConfig,
} from './types';

function textForOption<T extends string>(
  option: IbaseSelectSingleOption<T>,
): string {
...
function matchesRule<T extends string>(
  option: IbaseSelectSingleOption<T>,
  query: string,
  rule: IbaseSelectSingleSearchRule,
): boolean {
  const source =
    rule.key === 'label'
...
function resolveVirtualConfig(
  virtual: IbaseSelectSingleProps['virtual'],
): Required<IbaseSelectSingleVirtualConfig> | null {
...
export function IbaseSelectSingle<T extends string = string>({
  ariaLabel,
  ...
  const filteredOptions = useMemo(() => {
    ...
    if (info && order)
      return order
        ...
        .filter((option): option is IbaseSelectSingleOption<T> =>
          Boolean(option),
        );
    return indexes
