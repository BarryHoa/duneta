...
import type {
  TenoraSelectSingleOption,
  TenoraSelectSingleProps,
  TenoraSelectSingleSearchRule,
  TenoraSelectSingleVirtualConfig,
} from './types';

function textForOption<T extends string>(
  option: TenoraSelectSingleOption<T>,
): string {
...
function matchesRule<T extends string>(
  option: TenoraSelectSingleOption<T>,
  query: string,
  rule: TenoraSelectSingleSearchRule,
): boolean {
  const source =
    rule.key === 'label'
...
function resolveVirtualConfig(
  virtual: TenoraSelectSingleProps['virtual'],
): Required<TenoraSelectSingleVirtualConfig> | null {
...
export function TenoraSelectSingle<T extends string = string>({
  ariaLabel,
  ...
  const filteredOptions = useMemo(() => {
    ...
    if (info && order)
      return order
        ...
        .filter((option): option is TenoraSelectSingleOption<T> =>
          Boolean(option),
        );
    return indexes
