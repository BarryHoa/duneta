    ...
    <main className="min-h-screen overflow-hidden bg-[#07111f] px-5 pb-12 pt-5 text-slate-100 sm:px-8">
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <section id="modules" className="py-8">
          <Card className="border border-white/10 bg-white/[0.035]"><Card.Content className="grid gap-6 p-7 md:grid-cols-[1fr_auto]"><div><p className="text-xs font-semibold tracking-[0.18em] text-cyan-200">CLIENT MODULES</p><Card.Title className="mt-3 text-2xl font-semibold text-white">One UI package. Clear entrypoints.</Card.Title><Card.Description className="mt-3 max-w-2xl leading-7 text-slate-400">Use TenoraInput, TenoraSelect, TenoraForm, TenoraUpload, TenoraTable, TenoraError and TenoraLayout independently—or import them all from the shared client barrel.</Card.Description></div><div className="flex flex-wrap content-start gap-2 md:max-w-64">{['TenoraInput', 'TenoraSelect', 'TenoraForm', 'TenoraUpload', 'TenoraTable', 'TenoraLayout'].map((module) => <span key={module} className="rounded-lg border border-cyan-300/15 bg-cyan-300/[0.06] px-2.5 py-1.5 font-mono text-xs text-cyan-100">{module}</span>)}</div></Card.Content></Card>
        </section>
